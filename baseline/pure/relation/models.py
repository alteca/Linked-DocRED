...
class LongformerForRelation(LongformerModel):
    def __init__(self, config, num_rel_labels):
        super(LongformerForRelation, self).__init__(config)
        self.num_labels = num_rel_labels
...
class LongformerForRelationApprox(BertPreTrainedModel):
    def __init__(self, config, num_rel_labels):
        super(LongformerForRelationApprox, self).__init__(config)
        self.num_labels = num_rel_labels
        self.bert = LongformerModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.layer_norm = BertLayerNorm(config.hidden_size * 2)
