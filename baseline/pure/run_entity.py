    ...
    for i in range(len(batches)):
        ...
        for sample, preds in zip(batches[i], pred_ner):
            ...
            for span, pred in zip(sample['spans'], preds):
                span_id = '%s::%d::(%d,%d)'%(str(sample['doc_key']), sample['sentence_ix'], span[0]+off, span[1]+off)
                if pred == 0:
                    continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_longformer', action='store_true', 
                        help="whether to use Longformer model")
    
    parser.add_argument('--model', type=str, default='bert-base-uncased', 
                        help="the base model name (a huggingface model): ex. allenai/longformer-base-4096")
    ...
    if args.model == 'allenai/longformer-base-4096':
