...
def disambiguate_wikidata(surface_forms: list) -> dict:
    ...
    for i, surface_form in enumerate(tqdm(surface_forms)):
        ...
        response = requests.get(
            f'https://www.wikidata.org/w/api.php?action=wbsearchentities&search={search_encoded}&format=json&language=en&uselang=en&type=item')
...
def disambiguate_wikipedia(surface_forms: list) -> dict:
    ...
    for i, surface_form in enumerate(tqdm(surface_forms)):
        search_encoded = surface_form.replace("&", "%26")
        response = requests.get(
            f'https://en.wikipedia.org/w/api.php?action=opensearch&format=json&formatversion=2&search={search_encoded}&namespace=0&limit=10')
        if response.status_code == 200:
            content = json.loads(response.content)
...
def main(linked_docred_file: str, method: str, output_file: str):
    ...
    if method == 'wikipedia':
        disambiguated_surface_forms = disambiguate_wikipedia(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        prog='Entity-Linking with Wikipedia/Wikidata')
