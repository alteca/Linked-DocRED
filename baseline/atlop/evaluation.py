...
def to_official(preds, features, data_dir):
    rel2id = json.load(open(f'{data_dir}/meta/rel2id.json', 'r'))
...
def gen_train_facts(data_file_name, truth_dir):
    ...
    for data in ori_data:
        ...
        for label in data['relations']:
            rel = label['r']
            for n1 in vertexSet[label['h']]['mentions']:
                for n2 in vertexSet[label['t']]['mentions']:
...
def official_evaluate(tmp, path):
    ...
    for x in submission_answer:
        ...
        if (title, r, h_idx, t_idx) in std:
            ...
            for n1 in vertexSet[h_idx]['mentions']:
                for n2 in vertexSet[t_idx]['mentions']:
